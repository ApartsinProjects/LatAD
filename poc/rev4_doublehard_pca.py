...
Also reports the T2-only variant (filter B = T2 only). Outputs:
  _diagnostics/pca_filter_doublehard.json           all numbers
...
GDN_FILE = {"WADI_clean": "score_GDN_WADI_clean_s0.npy", "HAI": "gdn_fast_score_HAI_s0.npy",
            # clean official-normal GDN dump; the _diagnostics sibling is the leaked pre-fix dump (quarantined)
...
for name in DATASETS:
    ...
    EXTERNAL = ["IF", "AE", "LinRes", "USAD", "TranAD", "GDN"]
    ...
    json.dump(ALL, open(f"{OUT}/pca_filter_doublehard.json", "w"), indent=1)   # incremental persist
    log(f"   saved ({ALL[name]['seconds']}s)")

log(f"\nsaved -> {OUT}/pca_filter_doublehard.json")
