"""Recompute Table 5 (source-of-gain: cross-channel latent density vs channel-independent
marginal product) and Table 6 (head decomposition) for SWaT_canon on the CLEAN official
pipeline (0% train/test overlap). Verbatim arms/heads from clean_wadi_tables56.py /
e5_gain_clean.py; difficulty subset READ from the corrected scores_SWaT_canon.npz
(v2 coverage-stable axis -> 91 difficult windows). CFG per e5_gain_clean: K=40, LD=16.
HAI/WADI untouched.

Invariants stated in advance:
  I1  difficult subset == 91 windows (matches scores npz / OFFICIAL_v2 headline).
  I2  no arm scores below chance on the difficult subset (a <0.5 mean = a bug).
"""
...
NAME = "SWaT_canon"; K, LD = 40, 16; NSEEDS = 5
...
assert int(hard.sum()) == 91, f"I1 FAILED: difficult={int(hard.sum())} != 91"
print(f"{NAME}: difficult (v2 axis) = {int(hard.sum())} windows, normals = {int((y==0).sum())}, clip={clipv}, K={K}, LD={LD}, kd={kd}", flush=True)
...
for seed in range(NSEEDS):
    ...
    print(f"  seed {seed} done  (cc={cc[-1]:.3f} mp={mp[-1]:.3f})", flush=True)

cc, mp = np.array(cc), np.array(mp)
res = {"name": NAME, "n_difficult": int(hard.sum()), "n_normal": int((y == 0).sum()),
       "K": K, "latent_dim": LD, "k_density": kd, "clip": clipv, "nseeds": NSEEDS,
       ...
       "table6_heads": {h: {"auroc": round(float(np.mean(vv)), 3), "auroc_sd": round(float(np.std(vv)), 3)}
                        for h, vv in heads.items()}}
json.dump(res, open(os.path.join(OUT, "clean_swat_tables56.json"), "w"), indent=1)
print("\nTable 5 (SWaT clean, n_diff=%d):" % int(hard.sum()))
print(f"  cross-channel {cc.mean():.4f}+/-{cc.std():.4f}  marginal {mp.mean():.4f}+/-{mp.std():.4f}  gain {cc.mean()-mp.mean():+.4f}")
print("Table 6 heads (SWaT clean):")
for h in ["recon", "density", "nearest", "base", "base+resid", "LatAD"]:
    print(f"  {h:12} {np.mean(heads[h]):.3f}+/-{np.std(heads[h]):.3f}")
print("saved -> clean_swat_tables56.json", flush=True)
