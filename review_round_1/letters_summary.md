...
- Difficult-subset AUROC: HAI 0.845 (+0.088, CI [0.042,0.157], P about 0, significant); SWaT 0.837
  (+0.055, CI [0.012,0.104], P=0.007, significant); WADI 0.771 (+0.020, CI [-0.161,0.205], P=0.46, 8
  episodes, not significant, ties linear baseline, leads every learned detector).
...
- Dataset-scale scholarship: Intro, new Section 2.5 "Benchmark scale", Section 7, new refs (MetroPT 54,
  BATADAL 55, N-CMAPSS 56, BDG2 57, TSB-UAD 58).
...
| 4. Time-aware test all three + WADI CI/P | Episode-block bootstrap uniform on all three; CI/P defined; HAI/SWaT significant, WADI +0.020 P=0.46 (8 ep) stated as not separable | Section 6, Table 4, Figure 3 |
...
## Notes / open items carried from manuscript_reconcile.md (not blocking the letters)
- Table 2 (WADI single-model decomposition) still on pre-clean numbers; letters do not cite it.
- GDN double-hard cells are "-" (single-seed rescore not re-aggregated); letters say GDN is in Table 3.
- New refs appended [54]-[58], MDPI first-appearance renumber to run at build time.
